...
class Display:
    ...
    def task_list(tasks: list[Task], total: int, completed: int) -> None:
        ...
        if not tasks:
            ...
            print(f"{'ID':<10}{'STATUS':<10}{'TITLE':<60}")
            print(Display.LINE)

            for task in tasks:
                status = "[x]" if task.completed else "[ ]"
                title = task.title[:57] + "..." if len(task.title) > 60 else task.title
                print(f"{task.short_id:<10}{status:<10}{title:<60}")
    ...
    def task_details(task: Task) -> None:
        ...
        print(f"Created:     {task.created_at.replace('T', ' ')[:19]}")
        print(f"Updated:     {task.updated_at.replace('T', ' ')[:19]}")
        print(Display.SEPARATOR)
